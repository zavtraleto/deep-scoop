...
export class EraseParticles {
  ...
  spawn(from: Vec2, player: Vec2, r: number, g: number, b: number): void {
    const i = this.next;
  ...
  update(dt: number, player: Vec2): void {
    for (let i = 0; i < MAX; i++) {
      ...
      // Сначала разлёт, затем нарастающее притяжение к существу.
      const pull = Math.min(1, this.age[i] / 0.25) * 45;
      ...
      this.size[i] = this.baseSize[i] * Math.min(1, dist / 1.2 + 0.35);
    }
